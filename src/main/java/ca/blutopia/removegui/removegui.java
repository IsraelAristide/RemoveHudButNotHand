...
public class removegui {
    ...
    private void setup(final FMLCommonSetupEvent event) {
        ...
        defguiscale = Minecraft.getInstance().gameSettings.guiScale;
    }
    ...
    public void GuiScreen(GuiScreenEvent event) {
        ...
        if (ModConfigHolder.COMMON.toggleMod.get()) {
            ...
            if (gui != null) {
                if (window.getGuiScaleFactor() == 0.0) {
                    try {
    ...
    public void keyDown(InputEvent.KeyInputEvent event) {
        ...
        if (key == keynmap2.getKey().getKeyCode()) {
            if (action == 0) {
                Minecraft.getInstance().displayGuiScreen(new configScreen());
            }
        }

        if (key == keynmap.getKey().getKeyCode()) {
            if (action == 0) {
                if (!ModConfigHolder.COMMON.toggleMod.get()) {
